...
_test["load app details (json)"] = function()
	...
	_test.assert(util.equals(am.app.__get(), {
		configuration = {
			TEST_CONFIGURATION = {
				bool = true,
				bool2 = false,
...
_test["load app details missing default config (dev env)"] = function()
	...
	_test.assert(string.find(_log, "Failed to load default configuration", 0, true))
end
...
_test["load app details missing env config (dev env)"] = function()
	...
	_test.assert(string.find(_log, "Failed to load environment configuration", 0, true))
end
...
_test["remove app data (list of protected files)"] = function()
	am.options.CACHE_DIR = "tests/cache/2"
	local _testDir = "tests/tmp/app_test_remove_app_data"
	fs.mkdirp(_testDir)
	fs.remove(_testDir, { recurse = true, contentOnly = true })
	...
	_test.assert(_ok)
	local _ok, _entries = fs.safe_read_dir("data", { recurse = true })
	_test.assert(_ok and #_entries == 1)

	os.chdir(_defaultCwd)
end

_test["remove app data (keep function)"] = function()
	am.options.CACHE_DIR = "tests/cache/2"
	local _testDir = "tests/tmp/app_test_remove_app_data"
	fs.mkdirp(_testDir)
	fs.remove(_testDir, { recurse = true, contentOnly = true })
...
_test["remove app (list of protected files)"] = function()
	am.options.CACHE_DIR = "tests/cache/2"
	local _testDir = "tests/tmp/app_test_remove_app"
	fs.mkdirp(_testDir)
	fs.remove(_testDir, { recurse = true, contentOnly = true })
...
_test["remove app (keep function)"] = function()
	am.options.CACHE_DIR = "tests/cache/2"
	local _testDir = "tests/tmp/app_test_remove_app"
	fs.mkdirp(_testDir)
	fs.remove(_testDir, { recurse = true, contentOnly = true })
